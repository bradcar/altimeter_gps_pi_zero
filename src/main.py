...
from gps_utils import get_local_time, get_map_string, get_lat_string, get_lon_string
...
def display_altimeter_details(buzz, altitude_m, pressure_hpa, temp_c, humidity, iaq, is_metric, clock_string):
    epd_draw.rectangle((0, 0, 250, 122), fill=255)
...
def display_gps_details(gps, clock_string):
    if gps is not None:
        epd_draw.rectangle((0, 0, 250, 122), fill=255)
...
def display_big_dashboard(buzz, altitude_m, pressure_hpa, iaq, gps, is_metric):
    ...
    if gps is not None:
        ...
        print (f"lat-width={lat_str_width}, lon-width= {lon_str_width}")
...
def print_gps_metrics(gps: GPS, time_zone_hours: int):
    ...
    if gps is not None and gps.has_fix:
        ...
        if local_time and getattr(local_time, "tm_hour", None) is not None:
            print(
                "PDX DST timestamp: {}/{}/{} {:02}:{:02}:{:02}".format(
                    local_time.tm_mon,
                    local_time.tm_mday,
                    local_time.tm_year,
                    local_time.tm_hour,
    ...
    while True:
        ...
        if (current_time - last_gps_time) >= GPS_INTERVAL_SEC:
            ...
            if not gps.has_fix:
                ...
                clock_string = gps_clock_string(gps, time_zone_hours)
        ...
        if (current_time - last_eink_time) >= EINK_INTERVAL_SEC:
            ...
            if values_changed or button_pressed:
                ...
                buzzer_sound = None
                if show_env_details:
                    display_altimeter_details(buzzer_sound, altitude_m, pressure_hpa, temp_c, humidity, iaq, is_metric, clock_string)
